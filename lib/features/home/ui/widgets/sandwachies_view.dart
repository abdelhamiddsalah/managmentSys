...
class SandwachiesView extends StatelessWidget {
        ...
        builder: (context, state) {
          if (state is ProductsLoading) {
            ...
            return SingleChildScrollView(
              child: ItemsInSandwachiesPage (products: state.products),
            );
  ...
  class ItemsInSandwachiesPage extends StatelessWidget {
  final List<Product> products;
  const ItemsInSandwachiesPage({super.key, required this.products});
    ...
    shrinkWrap: true, // تحديد الحجم بناءً على المحتوى
     physics: NeverScrollableScrollPhysics(), // منع التمرير داخل GridView
      ...
      itemBuilder: (context, index) {
        return ItemInSandwaiches(product: products[index]);
      },
    );
  }
}